...
import { Card, Text, Title, Paragraph, Searchbar, useTheme, DataTable, Chip } from 'react-native-paper';
import AuditService from '../services/auditService';

export default function DashboardScreen({ inventory, onRefresh, isRefreshing = false }) {
  ...
  const mostUsedColor = useMemo(() => {
    ...
    auditLogs.forEach(log => {
      if (log.itemId && (log.action === 'check_in' || log.action === 'check_out' || log.action === 'update')) {
  ...
  const getActionColor = (action, details) => {
    ...
    switch (action) {
      case 'check_in': return '#81c784'; // Subtle green
      case 'check_out': return '#e57373'; // Subtle red
      case 'add': return '#64b5f6'; // Subtle blue
      case 'delete': return '#f44336';
      case 'update': return '#ba68c8'; // Subtle purple
  ...
  const getQuantity = (action, details, itemId) => {
    ...
    if (details) {
      // Handle old records with _actionType in details (for backward compatibility)
      const isCheckInOut = (action === 'check_in' || action === 'check_out') || 
                           (action === 'update' && details._actionType && 
          ...
          {mostUsedColor && (
            <Card style={styles.statCard}>
              <Card.Content>
                ...
                <Title style={[styles.statValue, { fontSize: 18 }]} numberOfLines={1}>
                  {mostUsedColor.name}
                </Title>
                <Text style={styles.statSubtext}>{mostUsedColor.count} transactions</Text>
              </Card.Content>
            </Card>
          )}
        </View>
            ...
            {filteredLogs.length === 0 ? (
              ...
              <ScrollView horizontal style={styles.tableScroll}>
                <DataTable style={styles.dataTable}>
                  <DataTable.Header>
                    <DataTable.Title style={styles.timeCell}>Time</DataTable.Title>
                    <DataTable.Title style={styles.userCell}>User</DataTable.Title>
                    <DataTable.Title style={styles.qtyCell} numeric>Qty (gal)</DataTable.Title>
                    <DataTable.Title style={styles.actionCell}>Action</DataTable.Title>
                    <DataTable.Title style={styles.colorCell}>Color</DataTable.Title>
                    <DataTable.Title style={styles.totalCell} numeric>Total (gal)</DataTable.Title>
                  </DataTable.Header>

                  {filteredLogs.slice(0, 100).map((log, index) => {
                    const actionText = formatAction(log.action, log.details, log.itemId);
                    const quantity = getQuantity(log.action, log.details, log.itemId);
                    const colorName = getItemName(log.itemId);
                    const totalQuantity = getTotalQuantity(log.action, log.details, log.itemId);
                    ...
                    return (
                      <DataTable.Row key={index}>
                        ...
                        <DataTable.Cell style={styles.userCell}>
                          <Text style={[styles.userText, { color: theme.dark ? '#fff' : '#666' }]}>{log.userName || 'Unknown'}</Text>
                        </DataTable.Cell>
                        ...
                        <DataTable.Cell style={styles.actionCell}>
                          <Chip 
                            style={{ backgroundColor: getActionColor(log.action, log.details) + '20' }}
                            textStyle={{ color: getActionColor(log.action, log.details), fontSize: 11 }}
                          >
                            {actionText}
                          </Chip>
                        </DataTable.Cell>
                        <DataTable.Cell style={styles.colorCell}>
                          <Text style={[styles.itemNameText, { color: theme.dark ? '#fff' : undefined }]}>
                            {colorName}
                          </Text>
                        </DataTable.Cell>
